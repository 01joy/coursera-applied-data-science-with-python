...
def answer_two():
    ...
    return df_copy.loc[df_copy['Diff'].idxmax()].name
```
...
def answer_three():
    ...
    df_copy['Diff']=(df_copy['Gold']-df_copy['Gold.1'])/(df_copy['Gold']+df_copy['Gold.1'])
    return df_copy.loc[df_copy['Diff'].idxmax()].name
```
...
```python
def answer_five():
    return census_df.groupby('STNAME')['CTYNAME'].nunique().idxmax()
```
...
def answer_seven():
    ...
    df_copy['diff']=df_copy[columns].max(axis=1)-df_copy[columns].min(axis=1)
    return df_copy.loc[df_copy['diff'].idxmax()]['CTYNAME']
```
...
def answer_eight():
    df_copy=census_df.copy()
    df_copy=df_copy[df_copy['SUMLEV'] == 50]
    df_copy=df_copy[((df_copy['REGION']==1)|(df_copy['REGION']==2))&(df_copy['CTYNAME'].str[:10]=='Washington')&(df_copy['POPESTIMATE2015']>df_copy['POPESTIMATE2014'])]
